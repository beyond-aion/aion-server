...
public class MySQL5AbyssRankDAO extends AbyssRankDAO {
	...
	public AbyssRank loadAbyssRank(int playerId) {
		...
		try (Connection con = DatabaseFactory.getConnection(); PreparedStatement stmt = con.prepareStatement(SELECT_QUERY)) {
			...
			if (rs.next()) {
				...
				abyssRank = new AbyssRank(daily_ap, weekly_ap, ap, rank, daily_kill, weekly_kill, all_kill, max_rank, last_kill, last_ap,
						last_update, daily_gp, weekly_gp, gp, last_gp);
	...
	private boolean insertRank(int playerId, AbyssRank rank) {
	...
	private boolean updateRank(int playerId, AbyssRank rank) {
	...
	@Override
	public List<RankingListPlayer> loadRankingListPlayers() {
		List<RankingListPlayer> results = new ArrayList<>();
		try (Connection con = DatabaseFactory.getConnection(); PreparedStatement stmt = con.prepareStatement(SELECT_RANKING_LIST_PLAYERS)) {
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
	...
	@Override
	public List<RankingListLegion> loadRankingListLegions() {
		List<RankingListLegion> results = new ArrayList<>();
		try (Connection con = DatabaseFactory.getConnection(); PreparedStatement stmt = con.prepareStatement(SELECT_RANKING_LIST_LEGIONS)) {
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
	...
	private int loadLegionMemberCount(Connection con, int legionId) {
	...
	@Override
	public List<RankingListPlayerGp> loadRankingListPlayersGp(Race race) {
		try (Connection con = DatabaseFactory.getConnection(); PreparedStatement stmt = con.prepareStatement(SELECT_RANKING_LIST_PLAYERS_GP)) {
	...
	@Override
	public Map<Integer, Integer> loadApOfPlayersNotInRankingList(Race race, AbyssRankEnum minRank) {
		try (Connection con = DatabaseFactory.getConnection(); PreparedStatement stmt = con.prepareStatement(SELECT_UNRANKED_PLAYERS_AP)) {
			stmt.setString(1, race.toString());
	...
	@Override
	public void updateAbyssRank(int playerId, AbyssRankEnum rank) {
	...
	@Override
	public void updateRankingLists(int maxOfflineDays, int playerLimit, int legionLimit) {
