		...
		try {
			...
			if (cmd.equalsIgnoreCase("enable")) {
				if (mode.equalsIgnoreCase("ip")) {
					...
					PacketSendUtility.sendMessage(player, "You account was locked on " + player.getClientConnection().getIP()
						+ " ip. You will not be able to login on this account with different IP address more.");
				} else if (mode.equalsIgnoreCase("pc")) {
					LoginServer.getInstance().sendPacket(
						new SM_CHANGE_ALLOWED_HDD_SERIAL(player.getPlayerAccount().getId(), player.getClientConnection().getHddSerial()));
					PacketSendUtility.sendMessage(player,
						"You account was locked on PC which you are logged from now. You will not be able to login on this account from other PC more.");
				} else {
