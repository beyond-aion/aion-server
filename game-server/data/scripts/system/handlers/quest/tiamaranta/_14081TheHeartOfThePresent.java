	...
	public boolean onKillEvent(QuestEnv env) {
		...
		if (qs != null && qs.getStatus() == QuestStatus.START) {
			...
			if(targetId == 233869){//Source Protector
				QuestService.addNewSpawn(600030000, player.getInstanceId(), 218766, (float) player.getX(), (float) player.getY(), (float) player.getZ(), (byte) 95);//Fragmented Pyreogre
				return defaultOnKillEvent(env, 233869, 2, 3);
			}
			if (targetId == 218766){ //Fragmented Pyreogre
				QuestService.addNewSpawn(600030000, player.getInstanceId(), 702090, (float) player.getX(), (float) player.getY(), (float) player.getZ(), (byte) 95);//Exploding Rock
	...
	public HandlerResult onItemUseEvent(final QuestEnv env, Item item) {
		...
		if (qs != null && qs.getStatus() == QuestStatus.START) {
			if (player.isInsideZone(ZoneName.get("LDF4B_ITEMUSEAREA_Q10060A"))) {				
				if (var == 1) {
					QuestService.addNewSpawn(600030000, player.getInstanceId(), 702314, (float) player.getX(), (float) player.getY(), (float) player.getZ(), (byte) 95);
