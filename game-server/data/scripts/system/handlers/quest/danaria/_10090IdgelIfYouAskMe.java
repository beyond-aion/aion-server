...
public class _10090IdgelIfYouAskMe extends QuestHandler {
			...
			if (targetId == 800566) {
				...
				switch (dialog) {
					...
					case SETPRO7: {
						...
						removeQuestItem(env, 182215242, 1);
	...
	public boolean onEnterZoneEvent(QuestEnv env, ZoneName zoneName) {
		...
		if (qs != null && qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 5) {
			...
			QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 800818, 553.97f, 430.22f, 94.8f, (byte) 45);
			return true;
		}
		return false;
	}
	...
	public boolean onMovieEndEvent(QuestEnv env, int movieId) {
		...
		QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (qs != null && qs.getStatus() == QuestStatus.START) {
			QuestService.addNewSpawn(player.getWorldId(), player.getInstanceId(), 800817, 218.56f, 510, 153.22f, (byte) 66);
			return true;
		}
		return false;
	}
