...
public class VocolithAI2 extends GeneralNpcAI2 {
	...
	public boolean onDialogSelect(Player player, int dialogId, int questId, int extendedRewardIndex) {
		if (dialogId == 10000) {
			if (player.getInventory().getItemCountByItemId(185000216) == 0) {
				PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1402539)); //You need a relic of ancestors to activate the obelisk
				return false;
	...
	private void handleUsed(Player player) {
		if (player.getInventory().decreaseByItemId(185000216, 1)) {
			PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1402541)); //You used a relic of ancestors
			World.getInstance().getWorldMap(getOwner().getWorldId()).getMainWorldMapInstance().doOnAllPlayers(new Visitor<Player>() {
				@Override
				public void visit(Player player) {
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1402540)); //An ancient monster has appeared
				}
			});
			spawnRandomBoss();
		} else {
	...
	private void spawnRandomBoss() {
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				SpawnTemplate spawn = SpawnEngine.addNewSingleTimeSpawn(getOwner().getWorldId(), npcId, getOwner().getX(), getOwner().getY(), getOwner().getZ(), getOwner().getHeading());
