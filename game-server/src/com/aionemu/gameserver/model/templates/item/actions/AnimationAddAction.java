...
public class AnimationAddAction extends AbstractItemAction {
	...
	public List<Integer> getMotionIds() {
		List<Integer> ids = new ArrayList<>(5);
		for (Integer id : new Integer[]{idle, run, jump, rest, shop}) {
			if (id != null) {
				ids.add(id);
			}
		}
	...
	public boolean canAct(Player player, Item parentItem, Item targetItem, Object... params) {
		...
		//Permanent animations cannot be studied repeatedly.
		if (minutes == null) {
			Map<Integer, Motion> learned = player.getMotions().getMotions();
			for (Integer id : new Integer[]{idle, run, jump, rest, shop}) {
				if (id != null) {
					Motion existing = learned.get(id);
					if (existing != null && existing.getExpireTime() == 0) {
