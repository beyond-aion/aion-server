...
public class CommandsAccessService {
	...
	public void giveAccess(Player admin, int playerId, String command) {
		if(haveRigths(playerId, command)) {
			PacketSendUtility.sendMessage(admin, "This player already have access on command "+command);
			return;
		}
		...
		PacketSendUtility.sendMessage(admin, "Command was granted successfull.");
	}
	
	public void removeAccess(Player admin, int playerId, String command) {
		if(!haveRigths(playerId, command)) {
			PacketSendUtility.sendMessage(admin, "This player haven't access on command "+command);
			return;
		}
		List<String> commands = playersAccess.get(playerId);
		commands.remove(command);
		DAOManager.getDAO(CommandsAccessDAO.class).removeAccess(playerId, command);
		PacketSendUtility.sendMessage(admin, "Command access was removed successfull.");
	}
	...
	public boolean haveRigths(int playerId, String command) {
		if(!playersAccess.containsKey(playerId))
			return false;
		List<String> commands = playersAccess.get(playerId);
