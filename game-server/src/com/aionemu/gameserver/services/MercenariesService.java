	...
	private static List<VisibleObject> spawn(MercenaryZone zone) {
		...
		for (Spawn spawn : zone.getSpawns()) {
			for (SpawnSpotTemplate sst : spawn.getSpawnSpotTemplates()) {
				SpawnTemplate spawnTemplate = SpawnEngine.addNewSingleTimeSpawn(zone.getWorldId(), spawn.getNpcId(), sst.getX(), sst.getY(), sst.getZ(), sst.getHeading());
				VisibleObject newMerc = SpawnEngine.spawnObject(spawnTemplate, 1);
				mercs.add(newMerc);
				log.info("Spawning mercenary, npc id "+spawn.getNpcId()+", at "+zone.getWorldId()+" "+sst.getX()+" "+sst.getY()+" "+sst.getZ());
			}
		}
		return mercs;
	}
