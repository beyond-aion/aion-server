	...
	private static void processRequests() {
		if(isProcessing.compareAndSet(false, true)) {
			...
			for(WebshopRequest request : requests) {
				try {
					SystemMailService.getInstance().sendMail(request.getBuyerName(), request.getReceiverName(), "In Game Shop", "", request.getItemId(),	request.getItemCount(), 0, LetterType.BLACKCLOUD);
