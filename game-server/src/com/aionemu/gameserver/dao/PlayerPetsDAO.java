...
public class PlayerPetsDAO {
	...
	public static void saveFeedStatus(int petObjectId, int hungryLevel, int feedProgress, long reuseTime) {
		try {
			try (var con = DatabaseFactory.getConnection();
				 var stmt = con.prepareStatement("UPDATE player_pets SET hungry_level = ?, feed_progress = ?, reuse_time = ? WHERE id = ?")) {
	...
	public static void saveDopingBag(int petObjectId, PetDopingBag bag) {
		try (var con = DatabaseFactory.getConnection();
			 var stmt = con.prepareStatement("UPDATE player_pets SET dopings = ? WHERE id = ?")) {
			var itemIdsBuilder = new StringBuilder();
			itemIdsBuilder.append(bag.getFoodItem()).append(",").append(bag.getDrinkItem());
			for (int itemId : bag.getScrollsUsed()) {
				itemIdsBuilder.append(",").append(itemId);
			}
			stmt.setString(1, itemIdsBuilder.toString());
	...
	public static void setTime(int petObjectId, long time) {
		try {
			try (var con = DatabaseFactory.getConnection();
				 var stmt = con.prepareStatement("UPDATE player_pets SET reuse_time = ? WHERE id = ?")) {
				stmt.setLong(1, time);
				stmt.setInt(2, petObjectId);
				stmt.execute();
			}
	...
	public static void insertPlayerPet(Player player, PetCommonData petCommonData) {
		try {
			try (var con = DatabaseFactory.getConnection();
				 var stmt = con.prepareStatement("INSERT INTO player_pets(id, player_id, template_id, decoration, name, despawn_time, expire_time) VALUES(?, ?, ?, ?, ?, ?, ?)")) {
	...
	public static void removePlayerPet(int petObjectId) {
		try {
			try (var con = DatabaseFactory.getConnection(); PreparedStatement stmt = con.prepareStatement("DELETE FROM player_pets WHERE id = ?")) {
	...
	public static List<PetCommonData> getPlayerPets(Player player) {
		...
		try {
			try (var con = DatabaseFactory.getConnection();
				 var stmt = con.prepareStatement("SELECT * FROM player_pets WHERE player_id = ?")) {
				stmt.setInt(1, player.getObjectId());
				try (var rs = stmt.executeQuery()) {
					while (rs.next()) {
						var petCommonData = new PetCommonData(rs.getInt("id"), rs.getInt("template_id"), player.getObjectId(), rs.getInt("expire_time"));
						...
						if (petCommonData.getDopingBag() != null) {
							var dopings = rs.getString("dopings");
							if (dopings != null) {
						...
						var ts = rs.getTimestamp("despawn_time");
						if (ts == null) {
	...
	public static void updatePetName(PetCommonData petCommonData) {
		try {
			try (var con = DatabaseFactory.getConnection();
				 var stmt = con.prepareStatement("UPDATE player_pets SET name = ? WHERE id = ?")) {
	...
	public static void savePetMoodData(PetCommonData petCommonData) {
		try {
			try (var con = DatabaseFactory.getConnection();
				 var stmt = con.prepareStatement("UPDATE player_pets SET mood_started = ?, counter = ?, mood_cd_started = ?, gift_cd_started = ?, despawn_time = ? WHERE id = ?")) {
	...
	public static int[] getUsedIDs() {
		try (var con = DatabaseFactory.getConnection();
			 var stmt = con.prepareStatement("SELECT id FROM player_pets", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
