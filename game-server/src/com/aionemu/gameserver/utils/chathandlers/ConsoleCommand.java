	...
	boolean process(Player player, String text) {
		...
		if (!checkLevel(player)) {
			if (LoggingConfig.LOG_GMAUDIT)
				log.info("[CONSOLE COMMAND] > [Player: " + player.getName() + "] has tried to use the command " + getAlias()
		...
		if (LoggingConfig.LOG_GMAUDIT) {
			if (player.getTarget() != null && player.getTarget() instanceof Creature) {
				...
				log.info("[CONSOLE COMMAND] > [Name: " + player.getName() + "][Target : " + target.getName() + "]: " + text);
			}
			else
				log.info("[CONSOLE COMMAND] > [Name: " + player.getName() + "]: " + text);
		}
