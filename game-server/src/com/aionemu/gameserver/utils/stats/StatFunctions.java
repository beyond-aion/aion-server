...
public class StatFunctions {
	...
	public static int calculateAttackDamage(Creature attacker, Creature target, boolean isMainHand, SkillElement element) {
		...
		if (target instanceof Npc)
			return target.getAi2().modifyDamage(attacker, resultDamage);
	...
	public static int calculatePhysicalAttackDamage(Creature attacker, Creature target, boolean isMainHand) {
		...
		/**
		* TODO
		* just regrowth under official values
		* 
		*/
		resultDamage*= 1.2f;
	...
	public static int calculateMagicalSkillDamage(Creature speller, Creature target, int baseDamages,
		...
		if (target instanceof Npc) {
			return target.getAi2().modifyDamage(speller, (int) damages);
		}
	...
	public static boolean calculateFallDamage(Player player, float distance, boolean stoped) {
		...
		if (distance >= FallDamageConfig.MAXIMUM_DISTANCE_DAMAGE || !stoped) {
			...
			player.getLifeStats().reduceHp(SM_ATTACK_STATUS.TYPE.FALL_DAMAGE, player.getLifeStats().getMaxHp() + 1, 0, SM_ATTACK_STATUS.LOG.REGULAR, player);
			return true;
		}
