...
public class CM_MAC_ADDRESS extends AionClientPacket {
	...
	protected void runImpl() {
		if (BannedMacManager.getInstance().isBanned(macAddress)) {
			...
			this.getConnection().close();
			log.info("[MAC_AUDIT] Account:" + this.getConnection().getAccount().getName() + ", HDD Serial " + hddSerial + " ("
				+ this.getConnection().getIP() + ") was kicked due to hdd serial ban");
		} else {
			...
			public void run() {
				...
				if (LoggingConfig.LOG_ACCOUNTS_LOGIN)
					LoginServer.getInstance().sendPacket(
						new SM_ACCOUNT_LOGIN_LOG(account.getId(), NetworkConfig.GAMESERVER_ID, System.currentTimeMillis(), getConnection().getIP(), macAddress,
							hddSerial));
				// TEMP FIX
				...
				if (!HDDBanService.getInstance().isAllowed(account.getAllowedHddSerial(), hddSerial)) {
					if (AntiHackConfig.HDD_SERIAL_HACKED_ACCOUNTS_KICK) {
						...
						getConnection().close();
						log.info("[MAC_AUDIT] Account:" + account.getName() + ",  HDD Serial " + hddSerial + " (" + getConnection().getIP()
